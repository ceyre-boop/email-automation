...
### Part 1 — The poller (runs inside the Render web service)

There is no separate poller deployment. The Render web service (Part 2 below) runs the
poller in-process via APScheduler — `auto_poll` every 90s, plus independent jobs for
the draft queue (60s), auto-send (60s), guardian, stall-alarm, claim-reaper, and
several reconciliation jobs (see `backend/main.py`). A Render Cron Job (`render.yaml`'s
`poller-heartbeat`, every 5 min) pings `GET /cron/poll-inboxes` on the live service as
a watchdog nudge, not an independent poller. Each unread email is processed end-to-end:
triage → draft/archive/flag → save to Postgres.
...
#### Step 4 — Send the onboarding link to each talent
...
Their token is stored in Supabase automatically. The poller picks it up on its next 90s cycle.

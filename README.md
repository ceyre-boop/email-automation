...
| `GOOGLE_SHEETS_REFRESH_TOKEN` | Run `python scripts/generate_google_refresh_token.py` once, then paste the printed refresh token |
| `APP_BASE_URL` | `https://email-automation.onrender.com` |

`GOOGLE_SHEETS_REFRESH_TOKEN` is the preferred production path. The app still supports `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` and `GOOGLE_SHEETS_SERVICE_ACCOUNT_FILE` as a legacy fallback, but some Google setups hard-block the service-account JSON flow, so the refresh token is the default recommendation.
...
| `GOOGLE_SHEETS_REFRESH_TOKEN` | Same refresh token generated by `scripts/generate_google_refresh_token.py` |

If you intentionally want the legacy Sheets auth path, the backend can still fall back to `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` or `GOOGLE_SHEETS_SERVICE_ACCOUNT_FILE`, but you should only use that if the refresh-token setup is not available in your environment.
...
3. **Google Sheets** — use the OAuth refresh-token flow for Sheets access by running `python scripts/generate_google_refresh_token.py` once and storing the result as `GOOGLE_SHEETS_REFRESH_TOKEN`. Share Britney's SOP sheet and the master log sheet with the Google account used for that token. If your environment still supports service accounts, the backend can also use `GOOGLE_SHEETS_SERVICE_ACCOUNT_JSON` or `GOOGLE_SHEETS_SERVICE_ACCOUNT_FILE` as a legacy fallback.
